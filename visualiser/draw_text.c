...
int	draw_text(SDL_Renderer *renderer, t_visu *visu)
{
	...
	{
		SDL_Surface* surfaceMessage = TTF_RenderText_Blended(visu->font, ft_itoa(visu->pathlines[j].fromx), Gray);
		SDL_Texture* Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage); 
		SDL_Rect Message_rect; //create a rect
		Message_rect.x = visu->pathlines[j].fromx - 10;  //controls the rect's x coordinate 
		Message_rect.y = visu->pathlines[j].fromy + 2; // controls the rect's y coordinte
		Message_rect.w = surfaceMessage->w; // controls the width of the rect
		Message_rect.h = surfaceMessage->h; // controls the height of the rect
		SDL_RenderCopy(renderer, Message, NULL, &Message_rect);
		SDL_FreeSurface(surfaceMessage);
